...
bool solve(int* array_first, int array_size_first, int* array_second, int array_size_second) {
    int count = 0;

    for (int i = 0; i < array_size_first; ++i)
        count += is_exits_element_in_array(array_first, array_size_second, array_second[i]);

    return count == array_size_first && count == array_size_second;
}
...
int main() {
    ...
    bool all_equeals = solve(array_first, array_size_first, array_second, array_size_second);

    printf("%d", all_equeals);
